...
class MWS(object):
    ...
    def make_request(self, extra_data, method="GET", **kwargs):
        ...
        request_description = '&'.join(['%s=%s' % (k, urllib.parse.quote(str(params[k]), safe='-_.~').encode('utf-8')) for k in sorted(params)])
    ...
    def calc_signature(self, method, request_description):
        ...
        return base64.b64encode(hmac.new(str(self.secret_key).encode(), sig_data.encode(), hashlib.sha256).digest())
...
class MerchantFulfillment(MWS):
    ...
    def _build_shipment_request_details(
            ...
            confirmation_type, label_format='PDF', **kwargs
    ):
        ...
        :param dimensions: The package dimensions. (Length, Width, Height)
        ...
        shipment_request_dict = {
            ...
            'ShipmentRequestDetails.Weight.Value': weight,
            ...
            'ShipmentRequestDetails.ShipFromAddress.AddressLine1': origin_address['addressline1'],
            'ShipmentRequestDetails.ShipFromAddress.City': origin_address['city'],
            'ShipmentRequestDetails.ShipFromAddress.StateOrProvinceCode': origin_address['state'],
            ...
            'ShipmentRequestDetails.PackageDimensions.Length': dimensions['length'],
            'ShipmentRequestDetails.PackageDimensions.Width': dimensions['width'],
            'ShipmentRequestDetails.PackageDimensions.Height': dimensions['height'],
            ...
            'ShipmentRequestDetails.ShippingServiceOptions.DeclaredValue.Amount': declared_value,
            'ShipmentRequestDetails.ShippingServiceOptions.LabelFormat': label_format,
        }
        for index, (item_id, quantity) in enumerate(item_list, start=1):
            shipment_request_dict['ShipmentRequestDetails.ItemList.Item.%d.OrderItemId' % index] = item_id
            shipment_request_dict['ShipmentRequestDetails.ItemList.Item.%d.Quantity' % index] = quantity
    ...
    def create_shipment(
            self, amazon_order_id, origin_address, weight, weight_unit, ship_date,
            item_list, dimensions, dimensions_unit, declared_value, declared_currency, is_pickup,
            confirmation_type, label_format, shipping_service_id, **kwargs
    ):
        ...
        Note: Scheduled carrier pickup is available only in the US using Dynamex.
        :param confirmation_type: The delivery confirmation level.
        :param label_format: Desired Label Format.
        ...
        shipment_request_dict = self._build_shipment_request_details(
            amazon_order_id, origin_address, weight, weight_unit, ship_date,
            item_list, dimensions, dimensions_unit, declared_value, declared_currency, is_pickup,
            confirmation_type, label_format, **kwargs
        )
        shipment_request_dict['Action'] = 'CreateShipment'
        shipment_request_dict['ShippingServiceId'] = shipping_service_id
